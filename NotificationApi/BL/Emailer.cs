    ...
    public class Emailer
    {
        ...
        public EmailResponse SendEMail(EmailData ed)
        {
            ...
            if (ed.type == "email-verification")
            {
                ...
                html_body = PrepareMessage("1", ed.url, ed.key);
            }
            else if (ed.type == "password-reset")
            {
                subject = "Reset your Password";
                html_body = PrepareMessage("2", ed.url, ed.key, ed.name);
            }
            var fromAddress = new MailAddress(ed.From, "FOBOH");
        ...
        private string PrepareMessage(string type, string url = "", string key = "", string name = "")
        {
            string body = "";
            if (type == "1")
            {
